...
main (int argc, char** argv)
{
    ...
    if(argc!=3) {
	cout<<"usage: "<<argv[0]<<" numberOfUniquePositions pathTotestDirectory\n Data has to be in the \
    ...
    Tinverse = Eigen::AngleAxis<double>(-15*M_PI/180,Eigen::Vector3d::UnitZ());
	...
	for(int j=1; j<10; j++) {
	    ...
	    //FPFH fixed = nd1 = cloud[j-1], moving = nd2
	    //start timing
	    gettimeofday(&tv_start,NULL);
	    matchFPFH(cloud[j-1],cloud[j],T_FPFH);
	    //stop timing1
	    gettimeofday(&tv_end,NULL);
	    ...
	    cloud2 = lslgeneric::transformPointCloud(T_HIST,cloud[j-1]);
	    cloud3 = lslgeneric::transformPointCloud(T_FPFH,cloud[j-1]);

	    FILE *f = fopen(fname,"w");
	    fprintf(f,"#VRML V2.0 utf8\n");
	    //green = target
	    lslgeneric::writeToVRML(f,cloud[j-1],Eigen::Vector3d(0,1,0));
	    //red = init
    ...
    cout<<"=================================\n HISTOGRAM: \n SUCC: "
	<<N_TRUE_HIST<<" FAIL: "<<N_FALSE_HIST<<" avg time "<<msec_HIST/(nPos*10)<<" msec\n FPFH: \n SUCC: "
